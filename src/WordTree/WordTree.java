...
public class WordTree
{
	...
	public void insert(String word)
	{
        ...
        WordTreeNode initialNode = getPrefixNode(word);
        int stringSize = word.length();
        int index = initialNode.depth;  //  Determines which part of the word has to be added to the tree

        while(index < stringSize){
            char letter = word.charAt(index);
            initialNode = initialNode.createChild(letter);
            index++;
        }
        initialNode.setEndOfWord(true);
        //  ADD YOUR ABOVE HERE
	}
	...
	WordTreeNode getPrefixNode(String word)
	{
        ...
        for (int stringIndex = 0; stringIndex < stringSize; stringIndex++){
              //  Travel the tree until the next child is not initialized
            char letter = word.charAt(stringIndex);
            int letterCode = (int) letter;

            if (tempNode.children[letterCode] == null){
                return tempNode;
            } else {
                tempNode = tempNode.children[letterCode];
            }
        }
	...
	{  
		//   ADD YOUR CODE BELOW HERE
        String prefix = this.getPrefix(word);
        boolean isIncluded = true;
        int stringSize = word.length();


        for(int index = 0; index < stringSize; index++){
            if ((prefix.length() == 0) ||(word.charAt(index) != prefix.charAt(index)) || !getPrefixNode(word).endOfWord)
              //  Tests for empty strings, .equals(), endOfWord
            {
                isIncluded = false;
                break;
            }
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
        ...
        wordList = traverse(prefixNode, prefix);
        return wordList;
		//  ADD YOUR CODE ABOVE HERE
	}
	...
	public class WordTreeNode
	{
		...
		public String toString()
		{
            ...
            String prefix = "";
            WordTreeNode tempNode = this;

            while (tempNode.charInParent!= (char)0){

                prefix = tempNode.charInParent + prefix;
	...
	///// HELPER METHOD /////
    public boolean isEmpty(WordTreeNode node){
	      //  Checks if children is an empty array
	    boolean empty = true;
    ...
    public ArrayList<String> traverse(WordTreeNode node, String prefix){
        ArrayList<String> wordList = new ArrayList<String>();
        WordTreeNode currentNode = node;

        if (currentNode.endOfWord) {
            wordList.add(currentNode.toString());
        }
        if ((isEmpty(currentNode)) || (currentNode.charInParent == (char)0) && prefix.length() > 0){
              /*
                If prefix doesn't exist in WordTree, then it returns a null node, and so does
                a prefix that is an empty string. However, we want the null string to return
                the entire list of words contained in the WordTree, so this checks whether or not
                the null node belongs to a null string or a string that doesn't exist in the WordTree
              */
            return wordList;
        }
        ...
        for (int index = 0; index < 256; index++) {
            if (!(currentNode.children[index] == null)){
                wordList.addAll(traverse(currentNode.children[index], ""));
            }
        }
        return wordList;
    }
